...
class Client extends events_1.EventEmitter {
    constructor(socket, opts, stringifier, parser) {
        super();
    ...
    call(method, params) {
        ...
        const message = { id, noiceClientId: this.guid, method, params };
        ...
        return new Proxy({}, {
            get: (target, prop) => {
                ...
                if (prop === '__proto__' || prop === 'prototype') {
                    ...
                    target[prop] = (event, handler) => this.on(event, handler);
                }
...
class Server extends events_1.EventEmitter {
    constructor(server, opts, stringifier, parser) {
        super();
        this.stringifier = stringifier;
        this.parser = parser;
        ...
        return new Proxy({}, {
            get: (target, prop) => {
                ...
                if (prop === '__proto__' || prop === 'prototype') {
                    ...
                    target[prop] = (event, ...params) => this.notify(event, params);
                }
                else if (prop.substr(0, 4) === 'emit' && prop.length > 5) {
                    const method = prop[4].toLowerCase() + prop.substr(5);
                    target[prop] = (...params) => this.notify(`${prefix}${method}`, params);
